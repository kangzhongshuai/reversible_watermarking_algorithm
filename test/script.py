    ...
    def in_img(self, address=''):
        if (address == ''):
            ...
            for i in range(0,maxp):
                if(frequency[i]==0):
                    min1=i;
            for i in range(255,maxp,-1):
                if(frequency[i]==0):
                    min2=i;
            print(min1,min2)
            if(maxp-min1>min2-maxp):
               min=min2
            else:
               min=min1
#            for i in range(256):
#               if (frequency[i] > frequency[min] and frequency[i] < frequency[maxp]):
#                    min = i
            print(maxp, min)
            if (maxp > min):
                for i in range(min, maxp + 1):
                    col[i] = 'cyan'
            else:
                for i in range(maxp, min + 1):
                    col[i] = 'cyan'
            ...
            if (maxp < min):
                for i in range(height):
                    for j in range(width):
                        if (pix[i, j] < min and pix[i, j] > maxp):
                            temp[j][i] = 255
                            pix[i, j] += 1
            else:
                for i in range(height):
            ...
            self.image2.setPixmap(
                QPixmap('./temp/shift.png').scaled(640, 360, Qt.IgnoreAspectRatio, Qt.FastTransformation))
        else:
            im = Image.open(self.loc1)
            pix = im.load()
            height, width = im.size  # Get the width and hight of the image for iterating over
    ...
    def out_img(self, address='./temp/embeded.png'):
        ...
        for i in range(height):
            for j in range(width):
                ...
                if (pix[i, j] == self.maxp):
                    k2 += 1
                elif (pix[i, j] == self.maxp - 1 and self.min < self.maxp):
                    image[k1, k2] = 255
                    k2 += 1
                elif (pix[i, j] == self.maxp + 1 and self.min > self.maxp):
                    image[k1, k2] = 255
        ...
        # histogram shifting
        self.min -= self.flag
        if (self.maxp < self.min):
            for i in range(height):
                for j in range(width):
                    if (pix[i, j] <= self.min and pix[i, j] > self.maxp):
                        pix[i, j] -= 1
        else:
            for i in range(height):
                for j in range(width):
        ...
        self.g4.plot(x=x, y=frequency, color='lime',
                     title=('after restoration    ' + 'psnr=' + str(round(Psnr(self.loc1, './temp/restored.png'), 4))))
